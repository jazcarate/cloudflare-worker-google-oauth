...
As this application is running in Cloudflare Workers, both the real environment in the cloud and the dev environment generated by `wrangler dev` (or `npm run dev`) injects some variables into the global scope. These variables are typed in the `bindings.d.ts` file; and are generated by steps 2 and 3 from [README.md#Setup wrangler](https://github.com/jazcarate/cloudflare-worker-google-oauth#setup-wrangler).
